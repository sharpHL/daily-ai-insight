"""
XinZhiYuan (新智元) Collector (via FOLO)

Fetches AI news from XinZhiYuan (www.xinzhiyuan.com) via FOLO API.
"""
...
class XinZhiYuanCollector(FOLOBaseCollector):
    """Collect XinZhiYuan (新智元) news via FOLO"""

    def __init__(self):
        feed_id = os.getenv('XINZHIYUAN_FEED_ID', '')
        fetch_pages = int(os.getenv('XINZHIYUAN_FETCH_PAGES', '3'))
    ...
    def _transform_item(self, entry: dict, feed: dict) -> Dict[str, Any]:
        """Transform FOLO XinZhiYuan entry to unified format"""
        ...
        feed_title = feed_data.get('title', '新智元')
...
# Test function
async def test_xinzhiyuan_collector():
    """Test XinZhiYuan collector"""
    print("Testing XinZhiYuan (新智元) Collector...")

    feed_id = os.getenv('XINZHIYUAN_FEED_ID')
    if not feed_id:
        print("⚠️  XINZHIYUAN_FEED_ID not set, skipping test")
        return
...
if __name__ == '__main__':
    import asyncio
    asyncio.run(test_xinzhiyuan_collector())
