"""
Jiqizhixin (机器之心) Collector (via FOLO)

Fetches AI news from Jiqizhixin (www.jiqizhixin.com) via FOLO API.
"""
...
class JiqizhixinCollector(FOLOBaseCollector):
    """Collect Jiqizhixin (机器之心) news via FOLO"""

    def __init__(self):
        feed_id = os.getenv('JIQIZHIXIN_FEED_ID', '')
        fetch_pages = int(os.getenv('JIQIZHIXIN_FETCH_PAGES', '3'))
    ...
    def _transform_item(self, entry: dict, feed: dict) -> Dict[str, Any]:
        """Transform FOLO Jiqizhixin entry to unified format"""
        ...
        feed_title = feed_data.get('title', '机器之心')
...
# Test function
async def test_jiqizhixin_collector():
    """Test Jiqizhixin collector"""
    print("Testing Jiqizhixin (机器之心) Collector...")

    feed_id = os.getenv('JIQIZHIXIN_FEED_ID')
    if not feed_id:
        print("⚠️  JIQIZHIXIN_FEED_ID not set, skipping test")
        return
...
if __name__ == '__main__':
    import asyncio
    asyncio.run(test_jiqizhixin_collector())
